...
iptables -A INPUT -d 127.0.0.1/32  -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 22 -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport $osuuApacheport -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 8080 -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 443 -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 440 -j ACCEPT
iptables -A INPUT -p tcp -m tcp --dport 3389 -j ACCEPT
...
mysqladmin -u root password "$osuusqlpass" #创建数据库密码
mysql -u root -p$osuusqlpass -e "create database vpndata;" #创建vpndata数据表
#mysql -u root -p$osuusqlpass -e "drop database test;" #删除默认test数据库
...
function osuu04() {
#修改Apache端口  修改主机名
sed -i "s/#ServerName www.example.com:80/ServerName localhost:$osuuApacheport/g" /etc/httpd/conf/httpd.conf
sed -i "s/Listen 80/Listen $osuuApacheport/g" /etc/httpd/conf/httpd.conf
setenforce 0 >/dev/null 2>&1
...
sed -i "s/newpass/"$osuusqlpass"/g" /etc/openvpn/auth_config.conf
...
function web() {
#安装web云端
rm -rf /var/www/* && cd /var/www && wget -q ${http}${host}fas_web.zip
...
#导入数据库vpndata表数据
sed -i "s/osuufasadmin/"$osuuadminuser"/g" /var/www/vpndata.sql
sed -i "s/osuufaspass/"$osuuadminpass"/g" /var/www/vpndata.sql
sed -i "s/服务器IP/"$IP"/g" /var/www/vpndata.sql
mysql -uroot -p$osuusqlpass vpndata < /var/www/vpndata.sql
rm -rf /var/www/vpndata.sql
#修改后台名
mv /var/www/html/phpMyAdmin /var/www/html/$osuusqldizhi
sed -i "s/newpass/"$osuusqlpass"/g" /var/www/html/config.php
#添加本地随机密钥
...
sed -i 's/demo.dingd.cn:80/'${llwsIP}:${osuuApacheport}'/g' `grep demo.dingd.cn:80 -rl /APP/fas/smali/net/openvpn/openvpn/`
...
cp -rf /APP/fas_sign.apk /root/fasapp_by_osuu.apk
rm -rf /APP
if [ ! -f /root/fasapp_by_osuu.apk ]; then
echo
echo "筑梦FAS系统APP制作失败！"
echo
...
echo "请自行前往官方交流群获取手动对接APP源！"
echo
echo "www.osuu.net    欢迎你的加入！"
fi
}
function zhuji() {
	clear
	echo
	read -p "请输入本机数据库地址(localhost): " osuusqlip
	if [ -z "$osuusqlip" ];then
	osuusqlip=localhost
	fi
	
	echo
	read -p "请输入本机数据库端口(3306): " osuusqlport
	if [ -z "$osuusqlport" ];then
	osuusqlport=3306
	fi
	 
	echo
	read -p "请输入本机数据库账号(root): " osuusqluser
	if [ -z "$osuusqluser" ];then
	osuusqluser=root
	fi
	
	echo
	read -p "请输入本机数据库密码: " osuusqlpass
	if [ -z "$osuusqlpass" ];then
	osuusqlpass=
	fi
	...
	sleep 3
	SQL_RESULT=`mysql -h${osuusqlip} -P${osuusqlport} -u${osuusqluser} -p${osuusqlpass} -e quit 2>&1`;
	...
	iptables -A INPUT -p tcp -m tcp --dport $osuusqlport -j ACCEPT
	service iptables save >/dev/null 2>&1
	systemctl restart iptables.service >/dev/null 2>&1
	if [[ $? -eq 0 ]];then
	echo "" >/dev/null 2>&1
	...
	mysql -h${osuusqlip} -P${osuusqlport} -u${osuusqluser} -p${osuusqlpass} <<EOF
grant all privileges on *.* to '${osuusqluser}'@'%' identified by '${osuusqlpass}' with grant option;
flush privileges;
EOF
	systemctl restart mariadb.service >/dev/null 2>&1
	if [[ $? -eq 0 ]];then
...
function fuji() {
	clear
	echo
	read -p "请输入本机IP: " osuubenjiip
	if [ -z "$osuubenjiip" ];then
	osuubenjiip=
	fi
	
	echo
	read -p "请输入主机IP: " osuusqlip
	if [ -z "$osuusqlip" ];then
	osuusqlip=
	fi
	
	echo
	read -p "请输入主机数据库端口: " osuusqlport
	if [ -z "$osuusqlport" ];then
	osuusqlport=
	fi
	
	echo
	read -p "请输入主机数据库账号: " osuusqluser
	if [ -z "$osuusqluser" ];then
	osuusqluser=
	fi
	
	echo
	read -p "请输入主机数据库密码: " osuusqlpass
	if [ -z "$osuusqlpass" ];then
	osuusqlpass=
	fi
...
#请谨慎修改
#数据库地址
mysql_host='$osuusqlip'
#数据库用户
mysql_user='$osuusqluser'
#数据库密码
mysql_pass='$osuusqlpass'
#数据库端口
mysql_port='$osuusqlport'
#数据库表名
mysql_data=vpndata
#本机地址
address='$osuubenjiip'
...
define("_host_","'$osuusqlip'");
define("_user_","'$osuusqluser'");
define("_pass_","'$osuusqlpass'");
define("_port_","'$osuusqlport'");
define("_ov_","vpndata");
...
define("_i_","i");'>/var/www/html/config.php && chmod -R 0777 /var/www/html/config.php
	...
	sleep 5
	echo
	echo "已成功为您的系统进行负载！主机IP为："$osuusqlip"！"
	echo 
	echo "副机系统请前往shell控制台输入 unfas、unsql 关闭后台登录权限，以防被不法份子入侵系统！"
	echo
	echo "请您及时前往主机FAS后台管理添加本机，本机IP: "$osuubenjiip""
}
function menufuzai() {
	clear
	echo
...
echo "---------------------------------------------"
echo "恭喜，您已经安装完毕。"
echo "控制台: http://"$IP":"$osuuApacheport"/admin/"
echo "账号: "$osuuadminuser" 密码: "$osuuadminpass""
echo "控制台随机本地密钥: "$suiji""
echo "内置数据库管理: http://"$IP":"$osuuApacheport"/"$osuusqldizhi"/"
echo "---------------------------------------------"
echo "数据库账户: root   密码: "$osuusqlpass"      "
echo "代理控制台: http://"$IP":"$osuuApacheport"/daili"
echo "---------------------------------------------"
echo "常用指令: "
...
echo "---------------------------------------------"
echo "APP下载地址: http://"$IP":"$osuuApacheport"/fas.apk"
echo "---------------------------------------------"
exit;0
}
function infoapp() {
	...
	echo
	read -p "请设置APP端口（默认：1024）: " osuuApacheport
	if [ -z "$osuuApacheport" ];then
	osuuApacheport=1024
	fi
	echo -e "已设置APP端口为:\033[32m "$osuuApacheport"\033[0m"
...
function fashoutaijiance() {
	if [ $osuusqldizhi == phpMyAdmin ];then
	osuusqldizhi=`date +%s%N | md5sum | head -c 20 ; echo`;
	echo -e "系统检测到您输入的数据库后台地址为phpMyAdmin，为了您的服务器安全，系统已默认修改您的数据库后台地址为: \033[32m"$osuusqldizhi"\033[0m";
	echo
	sleep 3
	else
	echo "" >/dev/null 2>&1
	...
	if [ $osuusqldizhi == llws ];then
	osuusqldizhi=`date +%s%N | md5sum | head -c 20 ; echo`;
	echo -e "系统检测到您输入的数据库后台地址为llws，为了您的服务器安全，系统已默认修改您的数据库后台地址为: \033[32m"$osuusqldizhi"\033[0m";
	echo
	sleep 3
	else
	echo "" >/dev/null 2>&1
	...
	if [ $osuusqldizhi == sql ];then
	osuusqldizhi=`date +%s%N | md5sum | head -c 20 ; echo`;
	echo -e "系统检测到您输入的数据库后台地址为sql，为了您的服务器安全，系统已默认修改您的数据库后台地址为: \033[32m"$osuusqldizhi"\033[0m";
	echo
	sleep 3
	else
	echo "" >/dev/null 2>&1
	...
	if [ $osuusqldizhi == admin ];then
	osuusqldizhi=`date +%s%N | md5sum | head -c 20 ; echo`;
	echo -e "系统检测到您输入的数据库后台地址为admin，为了您的服务器安全，系统已默认修改您的数据库后台地址为: \033[32m"$osuusqldizhi"\033[0m";
	echo
	sleep 3
	else
	echo "" >/dev/null 2>&1
...
function infofas() {
	...
	echo
	sleep 1
	read -p "请设置后台账号(默认admin): " osuuadminuser
	if [ -z "$osuuadminuser" ];then
	osuuadminuser=admin
	fi
	echo -e "已设置后台账号为:\033[32m "$osuuadminuser"\033[0m"
	
	echo
	read -p "请设置后台密码(默认随机): " osuuadminpass
	if [ -z "$osuuadminpass" ];then
	osuuadminpass=`date +%s%N | md5sum | head -c 20 ; echo`;
	fi
	echo -e "已设置后台密码为:\033[32m "$osuuadminpass"\033[0m"
	
	echo
	read -p "请设置后台端口(默认1024): " osuuApacheport
	if [ -z "$osuuApacheport" ];then
	osuuApacheport=1024
	fi
	echo -e "已设置后台端口为:\033[32m http://"$IP":"$osuuApacheport"\033[0m"
	
	echo
	read -p "请设置数据库地址(默认随机,禁用phpMyAdmin): " osuusqldizhi
	if [ -z "$osuusqldizhi" ];then
	osuusqldizhi=`date +%s%N | md5sum | head -c 20 ; echo`;
	fi
	echo -e "已设置数据库地址为:\033[32m http://"$IP":"$osuuApacheport"/"$osuusqldizhi"\033[0m"
	
	echo
	read -p "请设置数据库密码(默认随机): " osuusqlpass
	if [ -z "$osuusqlpass" ];then
	osuusqlpass=`date +%s%N | md5sum | head -c 20 ; echo`;
	fi
	echo -e "已设置数据库密码为:\033[32m "$osuusqlpass"\033[0m"
...
function menu() {
	...
	echo -e "       镇江30M50G防御月付69  cloud.osuu.net     "
	echo -e "************************************************"
	echo -e "请选择："
	echo
...
version="006";
http=http://
host=raw.githubusercontent.com/Demontisa/ML/main/op/fas/
logo
}
main
exit;0
