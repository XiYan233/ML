...
hostfile=raw.githubusercontent.com/Demontisa/ML/main/op/qyun;
...
clear
echo -n -e "输入 [\033[32m $key \033[0m] ："
read PASSWD
readkey=$PASSWD
if [[ ${readkey%%\ *} == $key ]]
then
...
then
echo
wget https://raw.githubusercontent.com/oooldking/script/master/superspeed.sh >/dev/null 2>&1
...
wget -O /etc/yum.repos.d/CentOS-Base.repo ${https}${hostfile}/Centos-7.repo
rpm -ivh ${https}${hostfile}/epel-release-latest-7.noarch.rpm
rpm -Uvh ${https}${hostfile}/remi-release-7.rpm 
